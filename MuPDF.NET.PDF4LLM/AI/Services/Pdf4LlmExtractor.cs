...
using MuPDF.NET.PDF4LLM.AI.Abstractions;
using MuPDF.NET.PDF4LLM.AI.Models;

namespace MuPDF.NET.PDF4LLM.AI.Services;

/// <summary>Phase 1 extraction via MuPDF.NET.PDF4LLM <c>to_text(page_chunks=true)</c>.</summary>
public sealed class Pdf4LlmExtractor : IPdfExtractor
{
    private readonly bool _useLayout;

    /// <param name="useLayout">When <see langword="true"/>, extract using the MuPDF.NET.PDF4LLM layout pipeline.</param>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            MuPDF4LLM.SetUseLayout(_useLayout);

            string json = MuPDF4LLM.ToText(
                pdfPath,
                pageChunks: true,
                header: true,
                footer: true,
    ...
    private static List<ExtractedPage> ExtractLegacyPages(string pdfPath)
    {
        ...
        {
            string text = MuPDF4LLM.ToMarkdown(
                doc,
