...
- `Page.FindTables()` **/** `TableHelpers.FindTables()` **/** `Utils.GetTables()`: new options matching PyMuPDF
  - `useLayout` (default `true`) — gate/complete line-based detection with layout table boxes; `false` runs full line-based detection only
  - `union` (default `false`) — fuse layout analyzer grids with line-based candidates
  - `refine` (default `false`) — refine cell grids, resolve spans/headers, and attach placement metadata
- `Table`: optional reported bbox override; `Placements`, `HeaderRows`, `SectionRows`, and `ToHtml()` for the refine path
- New table helper modules: `Table.Union`, `Table.Refine`, `Table.Spans`, `Table.Headers` (ports of PyMuPDF `_table_*.py`)
- `Document.RewriteImages`: `setToGray` and `options` (`PdfImageRewriterOptions`) match PyMuPDF `Document.rewrite_images`. When `options` is set, other image-type flags are ignored (except `setToGray`). Legacy `RewriteImage` forwards to `RewriteImages`.



#### Bug fixes

- `Document.Scrub(hiddenText: …)` (#4670): remove hidden text (e.g. GlyphLessFont / zero-alpha / unfilled+unstroked spans) via redaction annotations, then apply redactions — works with typed `GetText("dict")` / `PageInfo` spans
- `LinkDest` **/** `Document.ResolveNames` **/ TOC destinations** (#5044): null-safe Fit*/XYZ parsing, more robust `#page` / `#view` / named-dest URI handling; `LinkDest(string uri, …)` constructor for tests and URI-only construction
- `Table.Bbox` **/ layout IoU filtering** (#5030): guard empty-cell tables so bbox does not throw on empty grids
- `Page.ClipToRect()`: for MuPDF ≥ 1.28.1, pass the clip in page coordinates (do not multiply by `TransformationMatrix`), matching PyMuPDF — fixes leftover text outside the clip rect
- `MakeTableFromBbox`: call `fz_find_table_within_bounds` with the MuPDF 1.28.1+ signature
- `Helpers.CalcImageMatrix` / `Page.InsertImage(keepProportion: true)`: square images (`fw == fh`) now fit uniformly inside the target rect instead of stretching to fill
- `Document.IsFormPdf`, `IsFormPDF`, and `FormFonts`: hardened against debugger Locals evaluation AVs on closed / non-PDF / Office documents (`IsClosed` / `IsPdf` guards + `[DebuggerBrowsable(Never)]`)



#### Tests

- Ported PyMuPDF 1.28.2 coverage into `MuPDF.NET.Test`, including `Test4670`, `Test5044`, annot/font/general/markdown/named-link/TOC cases, expanded `TestTables` (`use_layout` / `union` / `refine`), and reworked `TestClipPage.test_clip`
- Copied corresponding fixtures under `TestDocuments/MuPDF.NET.Test/`
- `TestRewriteImages`: `setToGray` and custom `options`; `TestInsertimage.test_keep_proportion_square`
