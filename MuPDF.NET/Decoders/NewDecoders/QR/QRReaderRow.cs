...
namespace BarcodeReader.Core.QR
{
    ...
    class QRReaderRow
    {
        ...
        private LinkedList<BarCodeRegion> candidates;//need to be synchronized!!

        Encoding encoding;

        public QRReaderRow(ImageScaner scan, BlackAndWhiteImage bwSourceImage, LinkedList<BarCodeRegion> candidates, Encoding encoding)
        {
            this.scan = scan;
            ...
            crossFinder = new PatternFinderNoiseRow(QRFinder.finder, true, true, 2);
            patternFinder2 = new PatternFinderNoise(bwSourceImage, QRFinder.finder[0], true, 2); //both share the same cache to speed up detection.
            crossFinder2 = new PatternFinderNoise(bwSourceImage, QRFinder.finder[0], true, 2); //both share the same cache to speed up detection.
            smallPatternFinder2 = new PatternFinderNoise(bwSourceImage, QRFinder.smallFinder[0], false, 2);
            smallCrossFinder2 = new PatternFinderNoise(bwSourceImage, QRFinder.smallFinder[0], false, 2);
        ...
        public void ScanRow(int y, XBitArray row)
        {
            ...
            LinkedList<Pattern> removedPatterns = Pattern.RemoveOldPatterns(foundPatterns, y);
        ...
        bool AlreadyExists(MyPoint center)
        {
            var node = candidates.First;
            while (node != null)
            {
                if (node.Value.In(center))
                    return true;
                node = node.Next;
            }
        ...
        private bool ProcessPattern(StackedPattern p)
        {
            ...
            if (SquareFinder.CheckCrossPattern(scan, a, b, center, crossFinder, factory, out pUp, out pDown))
            {
                ...
                if (finder != null)
                {
                    ...
                    var locations = QRLocation.Scan(scan, patternFinder2, crossFinder2, smallPatternFinder2, smallCrossFinder2, finder);
                    ...
                    if (data != null)
                    {
                        if (encoding != null)
                            foreach (ABarCodeData d in data)
                                if (d is Base256BarCodeData codeData)
                                    if (codeData.encoding == null)
                                        codeData.encoding = encoding;
        ...
        {
            confidence = 1F;
            if (!symbol.EnrichFormatInformation())
            {
                return null;
            }
            ...
            if (failsNumber >= blockCount)
            {
                //throw new Exception("ERROR: wrong QR parameters table");
                return null;
            }
        ...
        ABarCodeData[] DecodeData(QRSymbol symbol, BitArray dataStream)
        {
            ...
            {
                ddsData = new ABarCodeData[] { SymbolDecoder.DecodeNumeric((int) MicroQRVersion.M1, dataStream, null, ref index) };
                return null;
            }
            ...
            while (index < dataStream.Length && !QRUtils.IsStreamEnd(dataStream, index, symbol.Version))
            {
                ...
                if (decoderMap.ContainsKey(mode))
                {
                    ...
                    ABarCodeData newData = decoder(symbol.Version, dataStream, textEncoding, ref index);
                    if (newData != null)
                    {
                        if (mode == 7) // ECI
                        {
                            textEncoding = EncodingUtils.GetEncodingFromECI(newData.ToString());
