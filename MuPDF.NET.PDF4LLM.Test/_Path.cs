...
{
    /// <summary>Test document paths relative to the MuPDF.NET.PDF4LLM solution root.</summary>
    internal static class _Path
    {
        ...
        public static string ResolveTestDocument(string fileName, string? subFolder = null)
        {
            ...
            return string.IsNullOrWhiteSpace(subFolder)
                ? Path.Combine(root, "TestDocuments", "MuPDF.NET.PDF4LLM.Test", fileName)
                : Path.Combine(root, "TestDocuments", "MuPDF.NET.PDF4LLM.Test", subFolder, fileName);
        }
        ...
        public static string Office(string fileName) =>
            RequireTestDocument(fileName, "TestOffice");
        ...
        public static string ForOutput(string fileName, string testClassName)
        {
            ...
            string path = Path.Combine(
                ResolveSolutionRoot(), "TestDocuments", "MuPDF.NET.PDF4LLM.Test", OutputFolderName, testClassName, fileName);
            Directory.CreateDirectory(Path.GetDirectoryName(path)!);
            return path;
        }

        /// <summary>Test class fixture, or empty when absent.</summary>
        public static string ForTestClassOrUpstream(string fileName, string testClassName)
        {
            string local = Path.Combine(
                ResolveSolutionRoot(), "TestDocuments", "MuPDF.NET.PDF4LLM.Test", testClassName, fileName);
            if (File.Exists(local))
                return local;
            return "";
        }

        /// <summary>Returns <c>null</c> when the fixture is not present.</summary>
